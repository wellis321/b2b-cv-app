...
const validateProfileData = createValidator({
	...
	linkedin_url: [validators.url('LinkedIn URL'), validators.maxLength('LinkedIn URL', 250)],
	bio: [validators.maxLength('Bio', 500)],
	username: [
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		for (const field of ['full_name', 'location', 'username', 'bio', 'linkedin_url']) {
		...
		if (typeof profileData.bio === 'string') {
			profileData.bio = sanitizeInput(profileData.bio);
		}
		// Don't sanitize LinkedIn URL as it might break the URL structure
		...
		if ('photo_url' in profileData) {
			...
			if (profileData.photo_url !== null && typeof profileData.photo_url !== 'string') {
				safeLog(
					'warn',
					`[${requestId}] Invalid photo_url type: ${typeof profileData.photo_url}, value: ${String(profileData.photo_url).substring(0, 100)}`
				);
		...
		try {
			...
			if ('username' in profileData && profileData.username) {
				...
				if (
					...
					usernamePattern.test(profileData.username)
				) {
			...
			if ('email' in profileData && profileData.email !== undefined) {
				...
				if (
					session.user.email &&
					(!profileData.email || profileData.email === 'user@example.com')
				) {
					simplifiedData.email = session.user.email;
					safeLog(
						'info',
						`[${requestId}] Using authenticated email (${session.user.email}) instead of provided email (${profileData.email})`
					);
				} else {
			...
			if ('photo_url' in profileData) {
				if (profileData.photo_url === null) {
					...
					safeLog(
						'warn',
						`[${requestId}] photo_url is not a string or null, type: ${typeof profileData.photo_url}`
					);
				}
			}
			...
			if (updateError) {
				...
				if (updateError.code === 'PGRST301' || updateError.code === '42501') {
					...
					try {
						safeLog(
							'debug',
							`[${requestId}] Attempting profile update with admin client using simplified data:`,
							{
								data: simplifiedData
							}
						);
