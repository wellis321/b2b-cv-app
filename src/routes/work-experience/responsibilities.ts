...
export async function getResponsibilitiesForExperience(workExperienceId: string): Promise<CategoryWithItems[]> {
    if (!workExperienceId) {
        console.error('getResponsibilitiesForExperience called with invalid workExperienceId:', workExperienceId);
        return [];
    }
...
export async function addCategory(workExperienceId: string, name: string): Promise<ResponsibilityCategory | null> {
    try {
...
export async function addItem(categoryId: string, content: string): Promise<ResponsibilityItem | null> {
    try {
        ...
        console.log('Insert result:', { data, error, fullResult: insertResult });
