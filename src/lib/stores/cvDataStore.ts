...
async function loadUserCvData(userId: string, clientInstance?: any): Promise<CvData> {
	...
	try {
		...
		const workExperiencesResult = await client
			.from('work_experience')
			.select('*')
			.eq('profile_id', userId)
			.order('sort_order', { ascending: true })
		...
		const educationResult = await client
			.from('education')
			.select('*')
			.eq('profile_id', userId)
			.order('start_date', { ascending: false });
		...
		const certificationsResult = await client
			.from('certifications')
			.select('*')
			.eq('profile_id', userId)
		...
		const qualificationEquivalenceResult = await client
			.from('professional_qualification_equivalence')
			.select('*')
			.eq('profile_id', userId)
			.order('level', { ascending: true });

		const interestsResult = await client
			.from('interests')
			.select('*')
			.eq('profile_id', userId)
		...
		const professionalSummaryResult = await client
		...
		data.qualificationEquivalence = qualificationEquivalenceResult.error
			? []
			: qualificationEquivalenceResult.data || [];
		data.professionalSummary = professionalSummaryResult.error
			? null
			: professionalSummaryResult.data || null;
				...
				const workResponsibilities = await Promise.all(
					data.workExperiences.map(async (work) => {
						try {
							...
							work.responsibility_categories = categories.map(
								(cat: { id: string; name: string; sort_order: number }) => ({
									...cat,
									items: items.filter(
										(item: { category_id: string; content: string; sort_order: number }) =>
				...
				const qualificationWithEvidence = await Promise.all(
					data.qualificationEquivalence.map(async (qualification) => {
						try {
							...
							console.error(
								'Error fetching supporting evidence for qualification',
								qualification.id,
								':',
								error
							);
