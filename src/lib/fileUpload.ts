	...
	try {
		...
		try {
			...
			if (createError) {
				...
				// If this is an RLS error and the bucket was already listed, assume it exists but we can't create
				if (createError.message?.includes('row-level security policy')) {
					console.warn(
						`RLS policy prevented bucket creation for "${bucket}", but it might still be usable`
					);
					return {
						exists: true,
						error: `Note: RLS prevented modification: ${createError.message}`
					};
				}

				return { exists: false, error: `Failed to create bucket: ${createError.message}` };
			}

			console.log(`Created bucket "${bucket}" successfully`);
		} catch (createErr) {
			console.error(`Exception creating bucket "${bucket}":`, createErr);
			// Even if creation fails, the bucket might still be usable if it exists
			return {
				exists: bucketExists,
		...
		const { success, error: accessError } = await ensurePublicAccess(bucket);
		if (!success) {
			console.warn(`Bucket "${bucket}" created but public access setup failed:`, accessError);
