...
export const CV_SECTIONS: CVSection[] = [
	...
	},
	{
		...
		dataTable: 'professional_summary'
	},
	{
		id: 'work-experience',
	...
	},
	{
		...
		description:
			'Show how international and other qualifications are equivalent to local standards',
...
export function getAdjacentSections(path: string): {
	prev: CVSection | null;
	next: CVSection | null;
} {
...
export async function updateSectionStatus(): Promise<void> {
	...
	for (const section of CV_SECTIONS) {
		...
		try {
			...
			if (section.id === 'profile') {
			...
			} else if (section.id === 'professional-summary') {
				// Special handling for professional summary - check if it exists and has content
				...
				if (!summaryError) {
					const isComplete = !!summaryData && !!summaryData.description;
					sectionStatus.update((status) => {
						status[section.id] = {
							isComplete,
							count: isComplete ? 1 : 0
