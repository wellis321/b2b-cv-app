...
// Per-tab completion for profile (so users see what’s done)
$tabMainComplete = !empty(trim($profile['full_name'] ?? '')) && !empty(trim($profile['username'] ?? ''));
$tabPhotoComplete = !empty($profile['photo_url'] ?? '');
$tabColorsComplete = true; // Header colours always have a value (default or custom)
$tabVisibilityComplete = true; // Visibility always has a value
            ...
            <form method="POST" action="/profile.php" id="profile-form">
                ...
                <div class="p-6">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 bg-green-50 -mx-6 px-6 py-6 rounded-b-lg">
                        ...
                        <p class="text-xs text-green-800 mb-4">Add work experience, education, skills, projects, and more in one place.</p>
                        <a href="/content-editor.php" class="inline-flex items-center gap-2 px-5 py-3 bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 font-semibold shadow-sm">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            Edit My CV
                        </a>
                    </div>
